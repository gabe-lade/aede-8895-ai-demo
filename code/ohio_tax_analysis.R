...
# ============================================================================
# PART 2: Top 10 Counties by Districts at 20-Mill Floor
# ============================================================================

# Count districts at floor by county
county_floor_counts <- sd_data %>%
  filter(at_floor == TRUE) %>%
  count(county_name, name = "districts_at_floor") %>%
  arrange(desc(districts_at_floor)) %>%
  slice_head(n = 10) %>%
  mutate(county_name = fct_reorder(county_name, districts_at_floor))

# Create bar chart
p3 <- ggplot(county_floor_counts, aes(x = districts_at_floor, y = county_name)) +
  geom_col(fill = "#ba0c2f") +
  geom_text(aes(label = districts_at_floor), hjust = -0.3, size = 4) +
  scale_x_continuous(expand = expansion(mult = c(0, 0.15))) +
  labs(
    title = "Top 10 Counties by School Districts at 20-Mill Floor",
    subtitle = "These counties have the most districts affected by HB 186 revenue changes",
    x = "Number of Districts at 20-Mill Floor",
    y = NULL,
    caption = "Source: Ohio Department of Education, Tax Year 2024"
  ) +
  theme_minimal(base_size = 12) +
...
ggsave("output/top_counties_at_floor.png",
       p3, width = 10, height = 6, dpi = 300, bg = "white")

cat("\n✓ Top counties bar chart saved\n")
